...
class App(Tk):
    ...
    def guardar(self):
        selectordearchivo = filedialog.asksaveasfile(filetypes=(('text files', '*.txt'),('All files', '*.*')), defaultextentions='.txt',mode='w')
        if selectordearchivo is not None:
    ...
    def sobre(self):
        messagebox.showinfo(title='Sobre mi', message='Creado por: RA201803')
